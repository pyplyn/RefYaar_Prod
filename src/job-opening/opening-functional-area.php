        ...
        <table id="example" class="table-bordered table-hover table-responsive table table-striped table-bordered" cellspacing="0" width="100%">
            ...
            <tbody>
                ...
                if ($_GET["key"] == "Functional Area") {
                    ...
                    $queryFuncAreaProce = "call select_job(?)";
                    $stmtFuncAreaProce = $con->prepare($queryFuncAreaProce);
                    $stmtFuncAreaProce->bind_param('s', $_SESSION["social_id"]);
                    $stmtFuncAreaProce->execute();
                    $stmtFuncAreaProce->bind_result($funcAreaPostId, $funcArea, $organization, $headline, $minExp, $maxExp, $city, $status, $postTime);
                    $stmtFuncAreaProce->store_result();
                    while ($stmtFuncAreaProce->fetch()) {
                        $Query = "select c.id,e.area,c.organization,c.headline,c.min_req_exp,c.max_req_exp,c.status,c.post_time,d.city from job_post c,location__city d,func_area e where c.city_id=d.id  and c.func_area_id=e.id and e.area=? and c.id=?";
                        $stmt = $con3->prepare($Query);
                        $stmt->bind_param('si', $_GET["key"], $funcAreaPostId);
                        $stmt->execute();
